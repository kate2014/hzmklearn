...
import com.mockuai.usercenter.mop.api.action.UpdateMessageStatus;
...
public class UpdateMessageTest {
    ...
    public void test_deleteMessage(){
        UpdateMessageStatus updateMessageAction =
                (UpdateMessageStatus) actionHolderMock.getAction("/user/message/update");
        RequestMock requestMock = new RequestMock();
        requestMock.setParam("id","2");
        requestMock.setAttribute("user_id", 1L);
        MopResponse mopResponse = updateMessageAction.execute(requestMock);
