...
import com.mockuai.datacenter.common.constant.ParamEnum;
import com.mockuai.datacenter.common.constant.ResponseCode;
import com.mockuai.datacenter.core.exception.DataException;
import com.mockuai.datacenter.core.filter.FilterChain;
import com.mockuai.datacenter.core.filter.FilterManager;
import com.mockuai.datacenter.core.service.action.Action;
import com.mockuai.datacenter.core.service.action.ActionHolder;
...
public class RequestDispatcher {
	...
	public DataResponse dispatch(DataRequest req) {
		if (req == null) {
	...
	private String getAppCode(RequestContext context, DataRequest req) {
		// TODO impl
		String appCode = null;
	 ...
	 * </pre>
	 */
	class ActionCall implements Callable<DataResponse> {

		private DataRequest req;

		public ActionCall(DataRequest req) {
			super();
			this.req = req;
		}
		...
		public DataResponse call() {
			...
			if (null != action) {
				...
				DataResponse re = new DataResponse(true);
				// 获取appCode
				String appCode = getAppCode(context, req);
				if (!allowAccess(appCode, action)) {
					re = new DataResponse(ResponseCode.BASE_STATE_E_REQUEST_FORBBIDEN);
					return re;
				}
				...
				try {
					...
					res = new DataResponse(e.getCode(), e.getMessage());
					log.error("do action:" + req.getCommand() + " occur Exception:", e);
					return res;
				} catch (Exception e) {
					res = new DataResponse(ResponseCode.SYS_E_SERVICE_EXCEPTION);
					log.error("do action:" + req.getCommand() + " occur Exception:", e);
					return res;
				} finally {
				...
				DataResponse res = new DataResponse(ResponseCode.BASE_STATE_E_ACTION_NO_EXIST);
				return res;
			}
		}
